...
class DinoGame(Widget):
    ...
    def setup_ui(self, dt):
        ...
        self.start_menu = BoxLayout(orientation='vertical', size_hint=(None, None), size=(dp(280), dp(250)), spacing=dp(12))
        with self.start_menu.canvas.before:
            Color(0, 0, 0, 0.92)
            self.menu_bg = RoundedRectangle(size=self.start_menu.size, radius=[dp(20)])

        self.start_menu.add_widget(Label(text="🦕 DINO GAME 🦕", font_size=sp(24), color=(0.2, 0.8, 0.2, 1), size_hint=(1, 0.4), bold=True))
...
class Calculator(BoxLayout):
    def __init__(self, app, **kwargs):
        ...
        self.display = TextInput(
            ...
            font_size=sp(48 * app.size_factor),
            size_hint=(1, 0.18),
            padding=[dp(20), dp(20)],
    ...
    def create_buttons(self):
        ...
        for row in layout:
            row_layout = GridLayout(
                ...
                spacing=dp(10),
                size_hint=(1, None),
                height=dp(85 * self.app.size_factor)
            )
    ...
    def on_button(self, instance):
        ...
        if t == "C":
            ...
            try:
                ...
                self.display.background_color = (0.1, 0.2, 0.1, 1)
                Clock.schedule_once(lambda dt: setattr(self.display, 'background_color', (0.05, 0.05, 0.08, 1)), 0.1)

            except:
                self.display.text = "Error"
                self.waiting_for_new_number = True
                self.display.background_color = (0.4, 0.1, 0.1, 1)
                Clock.schedule_once(lambda dt: setattr(self.display, 'background_color', (0.05, 0.05, 0.08, 1)), 0.5)
    ...
    def resize_buttons(self):
        self.display.font_size = sp(48 * self.app.size_factor)
        for btn in self.buttons:
            btn.font_size = sp(24 * self.app.size_factor)
            btn.height = dp(85 * self.app.size_factor)
        for row in self.button_rows:
            row.height = dp(85 * self.app.size_factor)
...
class History(BoxLayout):
    def __init__(self, app, **kwargs):
        ...
        self.history_box = TextInput(readonly=True, font_size=sp(22), background_color=(0, 0, 0, 0), foreground_color=(1, 1, 1, 1), padding=[dp(15), dp(15)])
...
class Settings(BoxLayout):
    ...
    def update_ui(self):
        ...
        self.slider = Slider(min=0.6, max=1.4, value=self.app.size_factor, size_hint=(1, None), height=dp(35))
        self.slider.bind(value=self.change_size)
        self.container.add_widget(self.slider)
        self.size_value = Label(text=f"Scale: {self.app.size_factor:.2f}", color=(1, 1, 1, 1), size_hint=(1, None), height=dp(25), font_size=sp(14))
...
class MainApp(BoxLayout):
    def __init__(self, **kwargs):
        ...
        top = GridLayout(cols=3, spacing=dp(8), padding=dp(8), size_hint=(1, 0.08))

        self.calc_btn = IOSButton(text="Calc", bg_color=self.theme["top"], size_factor=0.75)
        self.calc_btn.bind(on_press=lambda x: self.show("calc"))
        self.hist_btn = IOSButton(text="Hist", bg_color=self.theme["top"], size_factor=0.75)
        self.hist_btn.bind(on_press=lambda x: self.show("hist"))
        self.set_btn = IOSButton(text="Sets", bg_color=self.theme["top"], size_factor=0.75)
    ...
    def update_all(self):
        ...
        for btn in [self.calc_btn, self.hist_btn, self.set_btn]:
            btn.update_theme(self.theme["top"], (1, 1, 1, 1), 0.75, self.theme.get("button_radius", 35))
